...
module Simulate = 
  ...
  let rec run ss es trace =
    ...
    let rec eval ss es =
      ...
      | None -> Printf.printf "%s" (Events.string_of_stats es) in 
  eval ss es

  let start n loss term config trace = 
  let para = 
    match config with
    | None -> Parameters.({n;term;loss})
    | Some filename -> Parameters.of_json filename in
  let config = C.parse_config "" in
    run (States.init (fun n -> C.init n config) para) (Events.init para) trace
